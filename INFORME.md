...
- `nucleo/proyecto.py`: agregué `ID_CASO_RE` al lado de `ID_MEDIDA_RE`. La forma aceptada sale del corpus: `NNN-descripcion`, con minúsculas ASCII, dígitos y guiones.
- `nucleo/caso.py`: el lector `.caso`, el impresor y `cargar_fuente_caso()` rechazan ids de caso fuera de esa gramática. Esto cierra también el camino JSON.
- `tools/sintaxis.py`: `--verificar` recorre el corpus con el lector/impresor de casos, suma `casos convertidos: N` y falla si la ida y vuelta de casos no conserva JSON y texto canónico.
- `tools/metamorficas.py`: agregué dos sensores: ida y vuelta sobre todo el corpus real, y cobertura generada de la forma de caso. El generador cubre relación ausente, relación vacía, 1/2/3 relaciones, filas homogéneas y heterogéneas, clave declarada y ausente, texto, entero, float, `true`, `false`, `null`, texto `"null"`, prosa con backticks/comillas/saltos y `medida: null`.
- `catalogos/meta/meta.sintaxis_casos_ida_y_vuelta.oracle`: medida nueva para juzgar la reversibilidad del corpus real.
- `catalogos/meta/meta.sintaxis_casos_cubre_casos.oracle`: medida nueva para juzgar la reversibilidad de casos generados desde la forma de datos.
- `corpus/meta/127-sintaxis-casos-no-vuelve-igual.caso` y `128-sintaxis-casos-vuelve-exacta.caso`: polaridades de la ida y vuelta de casos reales.
- `corpus/meta/129-sintaxis-casos-generados-no-vuelve-igual.caso` y `130-sintaxis-casos-generados-vuelve-exacta.caso`: polaridades de la cobertura generada.
- `corpus/meta/131-sintaxis-casos-un-campo-por-vez.caso` y `132-sintaxis-casos-generados-un-campo-por-vez.caso`: fijan las disyunciones campo por campo para que `mutar.py` no deje sobrevivientes.
- `tools/corpus.py`: agregué `--nuevo grupo/NNN-descripcion`, con plantilla `.caso` parseable y cargable. También valida grupo e id antes de escribir.
- `tests/test_sintaxis.py`: agregué regresiones para gramática de ids de caso, conteo de casos en `--verificar`, sensores metamórficos de casos y andamio de caso.
- `README.md`: actualizado con `python tools/cifras.py --actualizar` porque cambiaron medidas, casos, mutantes y conteos publicados.
- `INFORME.md`: este informe.

## Experimento de rotura

Rotura temporal aplicada en `nucleo/caso.py`: `_escalar("null")` devolvía `null` sin comillas. Eso convierte el texto `"null"` en nulo JSON al imprimir. Después de esta corrida revertí la rotura.
...
juzgado por las medidas aplicables:
  ...
  ✗ meta.sintaxis_casos_cubre_casos                     5 (<= 0)
      → e={'propiedad': 'sintaxis_casos_cubre_casos', 'caso': '900-generado-relacion-ausente', 'origen': 'construido', 'evaluo': True, 'error': '', 'mismo_veredicto': False, 'mismo_valor': True, 'mismos_testigos': True}; e={'propiedad': 'sintaxis_casos_cubre_casos', 'caso': '901-generado-1-relaciones', 'origen': 'construido', 'evaluo': True, 'error': '', 'mismo_veredicto': False, 'mismo_valor': True, 'mismos_testigos': True}; e={'propiedad': 'sintaxis_casos_cubre_casos', 'caso': '902-generado-2-relaciones', 'origen': 'construido', 'evaluo': True, 'error': '', 'mismo_veredicto': False, 'mismo_valor': True, 'mismos_testigos': True} +2
...
```text
CIFRAS OK
  cifras: 518 tests · 535/535 mutantes de medida · **2247 sitios de mutación de código** (2042 + 205 del motor Python).
...
- No cambié la superficie de casos: no agregué campos, nodos, azúcar ni reglas nuevas de sintaxis.
- No migré los dos casos JSON existentes; el corpus sigue ejercitando los dos lectores.
- No toqué `ESCRIBIR-UNA-MEDIDA.md`, `ORACLE-TUTORIAL-PRACTICO.md` ni `vendor/`.
- No dejé aplicada ninguna rotura del experimento.
- No incorporé `DOCTRINA.md` ni `TAREA.md` al commit: estaban como archivos de consigna no versionados al empezar.

## Lo que descubrí

- La consigna hablaba de 91 `.caso`, pero la gramática se derivaba de 93 ids existentes porque el corpus también tenía 2 casos `.json`. Con los agregados quedaron 99 casos.
- Romper el impresor para omitir relaciones vacías ponía rojas las dos propiedades nuevas, porque el corpus real ya tenía relaciones vacías. Para demostrar una roja y la otra verde usé el borde `"null"` texto contra `null` JSON, que vive en el generador y no en el corpus real.
- Los primeros casos rojos de las medidas nuevas no alcanzaban para mutación: mataban la conducta general, pero no fijaban cada campo de la disyunción ni el borde de `aflojar_umbral`. Por eso separé casos de valor 1 y casos “un campo por vez”.
