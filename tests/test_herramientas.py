...
class HerramientasCLITests(unittest.TestCase):
    def test_un_objetivo_fuera_del_perfil_se_informa_sin_traceback(self):
        """El comando del relevo dejó escapar ValueError antes de entrar al manejo de errores."""
        ...
        for hechos in (False, True):
            with redirect_stdout(io.StringIO()) as salida, \
                    ...
                    mock.patch.object(mutar_codigo, "correr") as correr:
                codigo = mutar_codigo.main([
                    "--objetivo", "tools/no_existe.py", *(["--hechos"] if hechos else [])])
            self.assertEqual(codigo, 2)
            correr.assert_not_called()
            if hechos:
                error = json.loads(salida.getvalue())["error_mutacion"][0]
                self.assertEqual(error["tipo"], "ValueError")
                self.assertIn("tools/no_existe.py", error["mensaje"])
            else:
                self.assertIn("tools/no_existe.py", errores.getvalue())
            self.assertNotIn("Traceback", salida.getvalue() + errores.getvalue())
...
"""
    ...
    def test_la_mutacion_de_codigo_incluye_el_perfil_y_particiona_sin_escapar(self) -> None:
        """El perfil incluye las ubicaciones de error antes del descubrimiento general, sin escapar de la raíz."""
...
class ClasificacionDelCorpus(unittest.TestCase):
    ...
    MEDIDA = pruebas_sombras.ModoSombra.MEDIDA_ROJA
