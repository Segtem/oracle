...
def load_tests(loader, tests, pattern):
    """Conserva todos los casos, con el diagnóstico real después de las pruebas directas.

    Medido el 2026-09-09: reunir el diagnóstico real cuesta 0,64 s en estos tres tests.
    Con `failfast` un mutante cuesta lo que se tarda en llegar al test que lo mata.
    Aceptación y empaquetado viven en `test_cli_integracion`, al final del perfil.
    """
    diferidos = {
        "test_con_salida_lo_escribe_y_avisa_que_hay_que_leerlo": 1,
        "test_la_ruta_es_el_argumento_siguiente_a_la_bandera": 1,
        "test_sin_salida_lo_imprime_como_json": 1,
    }
    ...
    return unittest.TestSuite(sorted(
        casos(tests), key=lambda caso: diferidos.get(caso.id().rsplit(".", 1)[-1], 0)))
