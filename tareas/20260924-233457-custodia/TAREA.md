...
Implementar la Fase 2 según `DISENO-FASE2.md`: (1) incorporar la excepción de arnés testigo inmutable en `perfiles/python/mutacion_codigo.py:863-876` (`_comando_en_copia`); (2) escribir `tests/test_ejecutar_suite_mutacion.py` (suites sintéticas para protocolos 0/1/2, failfast y prioridades) y `tests/test_mutar_codigo_custodia.py` (esquema de equivalentes, reubicación y orquestación con mocks); (3) sumar ambos módulos a `HERRAMIENTAS_CUSTODIAS`, `PRIORIDADES` y `.github/workflows/verificar.yml`; (4) medir sitios reales, correr las rondas de mutación y cerrar mutantes vivos o registrar equivalentes. Mantener esta tarea ABIERTA para la Fase 3 (`tools/verificar_instalacion.py`).
